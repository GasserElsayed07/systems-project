...
export default function Home() {
        ...
        <div className="min-h-screen p-8">
          ...
          <div className="mb-8">
            <h1 className="text-4xl font-bold tracking-tight sm:text-5xl">
              <span className="text-violet-400">SIC/XE</span>{" "}
              <span className="text-white">Assembler</span>
            </h1>
            <p className="text-neutral-500 mt-2 text-base">
              Upload, write, or paste SIC/XE assembly code and assemble it in seconds.
            </p>
          </div>

          {/* Two-column layout */}
          <div className="grid grid-cols-1 lg:grid-cols-5 gap-8 items-start">
            {/* LEFT — tabs + active input */}
            ...
            {/* RIGHT — test files / quick-load options */}
                  ...
                  <div className="flex flex-col gap-2">
                    {testFiles.map((file: any, index: any) => (
                      <Button
                        key={index}
                        variant="outline"
                        className="justify-start h-auto py-2 border-neutral-800 hover:border-violet-800"
                        onClick={() => {
                      ...
                      >
                        <Code className="h-4 w-4 mr-2 text-violet-400 shrink-0" />
                        <span className="truncate">{file.title}</span>
                        <span className="text-xs text-neutral-500 ml-auto pl-2 shrink-0">
                          {file.description}
                        </span>
                      </Button>
                    ))}
  ...
  return (
    <div className="dark min-h-screen p-6 bg-[#07070e]">
      <div className="flex flex-wrap justify-between items-center mb-6">
        <h1 className="text-3xl font-bold">
          <span className="text-violet-400">Assembly</span>{" "}
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        {/* Left sidebar - symbol table & intermediate data */}
        ...
        {/* Right main area - object program & assembled code */}
        <div className="lg:col-span-2 space-y-6">
          {hteRecords && hteRecords.length > 0 && (
            <Card className="border border-violet-900/30">
              <CardHeader className="flex flex-row items-center justify-between pb-2">
                <CardTitle className="text-xl font-bold">
                  Object Program
                  <span className="ml-2 text-xs font-normal text-neutral-500 uppercase tracking-widest">HTME Records</span>
                </CardTitle>
                <Button
                  variant="outline"
                  size="sm"
                  onClick={downloadHTERecords}
                  className="ml-auto flex items-center gap-1 border-violet-900/40 hover:border-violet-700"
                >
